...
export const CelestialSphere = ({
  hue = 75.0,        // Lime/green hue to match neon sports palette
  speed = 0.3,
  zoom = 1.5,
  particleSize = 3.0,
  className = "",
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Final color calculation with neon sports palette
        float nebula = pow(t, 2.0);
        vec3 color = hsl2rgb(vec3(u_hue / 360.0 + nebula * 0.15, 0.65, 0.4));
        color *= nebula * 2.2;

        // Starfield with lime/green tint
        float star_val = random(vUv * 500.0);
        if (star_val > 0.998) {
            float star_brightness = (star_val - 0.998) / 0.002;
            vec3 star_color = hsl2rgb(vec3(u_hue / 360.0, 0.8, 0.6));
            color += star_color * star_brightness * u_particle_size;
        }
