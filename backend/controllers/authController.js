...
export const sendOtp = async (req, res) => {
  try {
    // add action for diffrefnt aaspact and check condition
...
export const completeSignup = async (req, res) => {
  try {
    ...
    if (exists) return res.status(400).json({ message: 'Contact number already registered' })
...
export const login = async (req, res) => {
  try {
    ...
    if (!user) return res.status(400).json({ message: 'Invalid contact number' })
